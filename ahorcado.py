...
class GameManager:
    ...
    def _render_frame(self):
        ...
        while True:
            ...
            if self.strikes >= 8:
                ...
                input('<Apretá ENTER para empezar devuelta o ctrl + c para salir>')
                self.strikes = 0
                self.word_indexes_used = []
                self._set_game_level()
                continue
            ...
            if len([letter for letter in self.word if letter not in self.guessed_letters]) == 0:
                    self._set_game_level()
                    input('<Te ganaste un chupetín salado!>')
                    continue
    ...
    def _render_level(self):
        ...
        print('=' * 15 + f'  NIVEL {len(self.word_indexes_used)}   ' + '=' * 15)
    ...
    def _load_words(self):
        ...
        with open(os.path.join(base_dir, './source/data/data.txt'), 'r', encoding='UTF-8') as file:
            for line in file:
 ...
 **   **  **   **  *******  **   **   *****   **   **  ******   *******
    ...
    def _ask_for_letter(self):
        ...
        print('Ingresa una letra y apretá ENTER.')
        input_letter = input(': ')

        if len(input_letter) != 1:
            print('Input error: LEÉ BIEN, MARMOTA. UNA LETRA.')
            input('<Después apretá ENTER>')
            return None
